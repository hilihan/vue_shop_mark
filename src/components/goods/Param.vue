    ...
    methods: {
      ...
      async getCateList() {
        ...
        if (res.meta.status !== 200) {
          return this.$message.error('获取商品类别失败！' + data.meta.msg);
        }
      ...
      async getParamsData() {
        ...
        if (res.meta.status !== 200) {
          return this.$message.error('获取商品参数失败！' + data.meta.msg);
        }
      ...
      addParams() {
        this.$refs.addFormRef.validate(async valid => {
          ...
          if (res.meta.status !== 201) {
            return this.$message.error('添加商品参数失败！' + data.meta.msg);
          }
          this.getParamsData();
      ...
      editParams() {
        this.$refs.editFormRef.validate(async valid => {
          ...
          if (res.meta.status !== 200) {
            return this.$message.error('修改商品参数失败！' + data.meta.msg);
          }
          this.getParamsData();
      ...
      async removeParams(attr_id) {
        ...
        if (res.meta.status !== 200) {
          return this.$message.error('删除商品参数失败！' + data.meta.msg);
        }
        this.getParamsData();
      ...
      showInput(row){
        ...
        //让文本框获得焦点
        //$nextTick 就是当页面上元素被重新渲染之后才会执行回调函数中的代码
        this.$nextTick(_ => {
      ...
      async saveAttrVals(row){
        ...
        this.$message.success('修改商品参数项成功！');
      }
    },
    computed: {
      //如果按钮需要被禁用则返回 true，反之返回false
      isBtnDisable() {
        if (this.selectedCateKeys.length !== 3) {
          return true;
        }
